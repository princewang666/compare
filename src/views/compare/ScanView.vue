<template>
    <div>
        <el-container  style="height: 730px; border: 1px solid #eee; box-shadow: 0 2px 4px rgba(0, 0, 0, .12), 0 0 6px rgba(0, 0, 0, .04)">
            <el-header style="font-size:40px; background-color: rgb(238, 241, 246)">全链路软解对比软件——扫描</el-header>
            <el-container>
                <!-- 侧边栏 -->
...
export default {
    ...
    // },
    // created() {
    //     // 界面创建后就构造一个定时器，定时发送刷新数据请求
    //     window.setInterval(() => {
    //         setTimeout(this.flushData(), 0);
    //     }, 10000);
    // },
}
</script>
